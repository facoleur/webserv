...
static bool isDirectiveKeyword(const std::string& s) {
    return s == "root" || s == "methods" || s == "return" || s == "location" || s == "host" || s == "listen" ||
           s == "error_page" || s == "autoindex" || s == "client_max_body_size" || s == "cgi" || s == "upload_enable" ||
           s == "upload_store";
}

std::vector<ConfigParser::Token> ConfigParser::tokenize(const std::string& text) {
    ...
    while (!accept("}")) {
        ...
        if (accept("host")) {
            ...
            if (ip.s == ";" || ip.s == "{" || ip.s == "}" || ip.s.empty()) {
                ...
                msg << "invalid host value '" << ip.s << "' at line " << ip.line << ", col " << ip.col;
...
void ConfigParser::parseLocation(ServerConfig& srv) {
    ...
    while (!accept("}")) {
        ...
        if (parseDirectiveAutoIndex(loc) || parseDirectiveClientMaxBodySize(loc) || parseDirectiveCgi(loc) ||
            parseDirectiveUploadEnable(loc) || parseDirectiveUploadStore(loc) || parseDirectiveRoot(loc) ||
            parseDirectiveIndex(loc) || parseDirectiveMethods(loc) || parseDirectiveReturn(loc)) {
            continue;
        }
        ...
        Token              t = next();
        std::ostringstream oss;
        oss << "unknown directive '" << t.s << "' in location (line " << t.line << ", col " << t.col << ")";
        throw ParseError(oss.str());
    }
...
bool ConfigParser::parseDirectiveRoot(ServerConfig& srv) {
    ...
    if (p.s == ";" || p.s == "{" || p.s == "}" || p.s.empty()) {
        std::ostringstream msg;
        msg << "invalid root value '" << p.s << "' at line " << p.line << ", col " << p.col;
        throw ParseError(msg.str());
    }
    srv.root = p.s;
...
bool ConfigParser::parseDirectiveMethods(ServerConfig& srv) {
    ...
    while (!accept(";")) {
        ...
        if (em == UNKNOWN)
            throw ParseError("invalid method '" + m.s + "' (allowed: GET POST DELETE)");
...
bool ConfigParser::parseDirectiveReturn(ServerConfig& srv) {
    ...
    Token tgt  = next();
    int   code = toInt(st.s);
    if (code <= 0 || tgt.s.empty() || tgt.s == "{" || tgt.s == "}" || tgt.s == ";")
